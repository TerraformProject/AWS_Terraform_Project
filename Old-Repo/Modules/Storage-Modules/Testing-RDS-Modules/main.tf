...
resource "aws_rds_cluster" "this" {
  ...
  count = var.create_cluster && var.create_db_subnet_group == true ? 1 : 0
  ...
  #Only valid when engine mode is set to serverless
  dynamic "scaling_configuration" {
    for_each = var.create_scaling_configuration == true ? [] : [var.scaling_configuration]
  ...
  dynamic "s3_import" {
    for_each = var.create_s3_import == true ? [var.s3_import] : []
    content {
  ...
  dynamic "restore_to_point_in_time" {
    for_each = var.create_restore_to_point_in_time == true ? var.restore_to_point_in_time : {}

    content{
    source_cluster_identifier  = lookup(var.restore_to_point_in_time, "restore_to_point_in_time", null)
    restore_type               = lookup(var.restore_to_point_in_time, "restore_type", "")
    use_latest_restorable_time = lookup(var.restore_to_point_in_time, "use_latest_restorable_time", "")
    restore_to_time = lookup(var.restore_to_point_in_time, "use_latest_restorable_time", "")
    }
  }
