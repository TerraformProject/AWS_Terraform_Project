...
resource "aws_network_acl" "private_acls" {
  ...
  vpc_id = each.value.vpc_id
  subnet_ids = each.value.private_acl_subnet_ids

 dynamic "ingress" {
      for_each = lookup(each.value, "private_network_acl_ingress", null)
      content {
        action          = ingress.value.action
        cidr_block      = ingress.value.cidr_block
        from_port       = ingress.value.from_port
  ...
  dynamic "egress" {
    for_each = lookup(each.value, "private_network_acl_egress", null)
    content {
      action            = egress.value.action
        cidr_block      = egress.value.cidr_block
        from_port       = egress.value.from_port
  ...
  tags = merge(
    {
      "Name" = each.value.private_network_acl_name
    },
    each.value.tags,
  )
}
...
resource "aws_network_acl" "database_acls" {
  for_each = var.database_network_acl 

  vpc_id = each.value.vpc_id
  subnet_ids = each.value.database_acl_subnet_ids

 dynamic "ingress" {
      for_each = lookup(each.value, "database_network_acl_ingress", null)
      content {
        action          = ingress.value.action
        cidr_block      = ingress.value.cidr_block
        from_port       = ingress.value.from_port
  ...
  dynamic "egress" {
    for_each = lookup(each.value, "database_network_acl_egress", null)
    content {
      action            = egress.value.action
        cidr_block      = egress.value.cidr_block
        from_port       = egress.value.from_port
  ...
  tags = merge(
    {
      "Name" = each.value.database_network_acl_name
    },
    each.value.tags,
  )
}